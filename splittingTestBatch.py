...
# ====== 使用者參數設定 ======
folder_path = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\1hz"
output_folder = os.path.join(folder_path, "output 1Hz new")
...
for index, filename in enumerate(image_files):
    ...
    # 計算逆時針旋轉 1 度的旋轉矩陣c
    rotation_matrix = cv2.getRotationMatrix2D(center, 3, 1)  # 1 代表旋轉 1 度，1 為縮放比例
    # 使用旋轉矩陣來旋轉圖片
    ...
    # 挑选主要边缘，生成黑白图像
    _, binary_image = cv2.threshold(sobel_edges, 40, 255, cv2.THRESH_BINARY)
    ...
    kernel_size = 3
    while cv2.contourArea(largest_contour) < 9000:
    ...
    # cv2.rectangle(electrode, (384,74), (692, 382), 255, -1)
    # overlap = cv2.bitwise_and(filled_droplet, electrode)
    subtract_mask = np.zeros_like(cropped_image)
    subtract_mask = 255 - subtract_mask
    cv2.rectangle(subtract_mask, (0,117), (140, 250), 0, -1)
