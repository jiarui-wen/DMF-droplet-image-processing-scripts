...
def find_2_largest_contours_from_binary(binary_img, filename):
    ...
    if len(valid_contours) >= 2:
        ...
        print(cv.contourArea(sorted_contours[0]), cv.contourArea(sorted_contours[1]))
        return sorted_contours[0], sorted_contours[1]
    elif len(valid_contours) == 1:
        # If only one valid contour, return it twice
        print(cv.contourArea(valid_contours[0]))
...
save_dir = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\splitting1kHz7.30\process images"
...
img_path = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\splitting1kHz7.30\Q2m 1291_00000003.jpg"
...
# 計算逆時針旋轉 1 度的旋轉矩陣c
rotation_matrix = cv.getRotationMatrix2D(center, 0, 1)  # 1 代表旋轉 1 度，1 為縮放比例
# 使用旋轉矩陣來旋轉圖片
rotated_image = cv.warpAffine(img, rotation_matrix, (w, h))


cv.imshow("rotated", rotated_image)
cv.imwrite(os.path.join(save_dir, '01_rotated.png'), rotated_image)
cv.waitKey(0)
cv.destroyAllWindows()

# 裁切區域：Y:170:620, X:250:1000 (420,90) (650,960)
...
# 挑选主要边缘，生成黑白图像
_, binary_image = cv.threshold(sobel_edges, 10, 255, cv.THRESH_BINARY)
...
largest_contour, second_largest_contour = find_2_largest_contours_from_binary(binary_image, img_path[-8:])
if cv.contourArea(largest_contour) > 60000:
    cv.drawContours(mask, [largest_contour], -1, (255, 255, 255), thickness=cv.FILLED)
else:
    cv.drawContours(mask, [largest_contour,second_largest_contour], -1, (255, 255, 255), thickness=cv.FILLED)

t = 1
kernel_size = 3
while not (cv.contourArea(largest_contour) > 60000 or (cv.contourArea(largest_contour) > 37000 and cv.contourArea(second_largest_contour) > 22000)):
    kernel = np.ones((kernel_size, kernel_size), np.uint8)
    mask = cv.morphologyEx(mask, cv.MORPH_CLOSE, kernel)
    largest_contour, second_largest_contour = find_2_largest_contours_from_binary(mask, img_path[-8:])
    letter = string.ascii_lowercase[t-1]  # 'a', 'b', 'c', ...
    cv.imshow("largest contour mask" + str(t), mask)
    cv.imwrite(os.path.join(save_dir, f'07{letter}_largest_contour_mask_filled_{t}.png'), mask)
    cv.waitKey(0)
    cv.destroyAllWindows()
    t += 1
    kernel_size += 2
    print(cv.contourArea(largest_contour), cv.contourArea(second_largest_contour))


# # 画最大封闭曲线的mask
# largest_contour_mask_filled = np.zeros_like(cropped_image)
# cv.drawContours(largest_contour_mask_filled, [largest_contour, second_largest_contour], -1, (255, 255, 255), thickness=cv.FILLED)
...
# t = 1
# kernel_size = 3
# while cv.contourArea(largest_contour) < 9000:
#     kernel = np.ones((kernel_size, kernel_size), np.uint8)
