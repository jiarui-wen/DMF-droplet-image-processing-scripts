...
# ====== 使用者參數設定 ======
folder_path = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\moving4.29\set2\500 Hz2 0"

image_files = [f for f in os.listdir(folder_path) if f.endswith(".tif")]
image_files.sort()

output_folder = os.path.join(folder_path, "output 500 Hz2 0")
os.makedirs(output_folder, exist_ok=True)

excel_output = os.path.join(output_folder, "500_Hz2_0.xlsx")

rotation_deg = 0  # 旋轉角度 (度)

crop_topleft = (13, 625)  # 裁切區域左上角 (X, Y)
crop_bottomright = (580, 930)  # 裁切區域右上角 (X, Y)
origin_uncrop = (crop_topleft[0], int((crop_bottomright[1]+crop_topleft[1])/2))  # 原始未裁切的原點 (X, Y)
subtract_mask_x_uncrop = 309  # 用於減去液滴的區域 X 座標
