...
output_folder = os.path.join(folder_path, "output 1 Hz 0-N001")
os.makedirs(output_folder, exist_ok=True)

excel_output = os.path.join(output_folder, "1_Hz_0-N001.xlsx")

rotation_deg = 0  # 旋轉角度 (度)

crop_topleft = (38, 560)  # 裁切區域左上角 (X, Y)
crop_bottomright = (605, 865)  # 裁切區域右上角 (X, Y)
origin_uncrop = (crop_topleft[0], int((crop_bottomright[1]+crop_topleft[1])/2))  # 原始未裁切的原點 (X, Y)
subtract_mask_x_uncrop = 336  # 用於減去液滴的區域 X 座標
...
print(f"Origin in cropped image: ({origin_x}, {origin_y})")

pixel_per_mm = (313-60)/0.4  # 每毫米多少像素
droplet_height_mm = 0.06 # 液滴高度 (毫米)
area_threshold = 66000  # 面積閾值，若液滴輪廓小于此值將被认为未框选完全

fps = 1000  # 每秒幀數
timestep = 1 / fps  # 每幀的時間間隔 (秒)
actuate_frame = 2903  # 开通电极的帧
...
data_list = []
for index, filename in enumerate(image_files[9071:9072]):
    ...
    # 挑选主要边缘，生成黑白图像
    _, binary_image = cv2.threshold(sobel_edges, 9, 255, cv2.THRESH_BINARY)
    ...
    # cv2.imshow("filled", filled_droplet)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
    ...
    # cv2.imshow("Subtract Mask", subtract_mask)
    cv2.rectangle(subtract_mask, (0,0), (subtract_mask_x, h), 0, -1)
