...
def find_largest_contour_from_binary(binary_img, filename):
    ...
    for cnt in cnts:
        if len(cnt) >= 3:
            ...
            if major_axis >= 10 and minor_axis >= 10:
...
# ====== 使用者參數設定 ======
folder_path = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\100 Hz\frames"
output_folder = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\100 Hz\output 100Hz"
...
for index, filename in enumerate(image_files):
    ...
    cropped_image = image[170:580, 20:770]
    ...
    img_contrast = cv2.convertScaleAbs(rotated_image, alpha=1.5, beta=0)

    # 模糊：用bilateral而不用gaussian，这样可以避免液滴边缘被模糊
    # blurred_image = cv2.GaussianBlur(rotated_image, (7, 7), 20)
    blurred_image = cv2.bilateralFilter(img_contrast, d=9, sigmaColor=75, sigmaSpace=15)
    ...
    # 挑选主要边缘，生成黑白图像
    _, binary_image = cv2.threshold(sobel_edges, 30, 255, cv2.THRESH_BINARY)

    contours, _ = cv2.findContours(binary_image, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    contours_mask = np.zeros_like(cropped_image)
    cv2.drawContours(contours_mask, contours, -1, (255, 255, 255), thickness=cv2.FILLED)
    contours_mask = cv2.erode(contours_mask, np.ones((5, 5), np.uint8), iterations=1)
    ...
    kernel_size = 3
    while cv2.contourArea(largest_contour) < 80000:
    ...
    contours, _ = cv2.findContours(filled_droplet, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    largest_contour = max(contours, key=cv2.contourArea)

    subtract_mask = np.zeros_like(cropped_image)
    subtract_mask = 255 - subtract_mask
    cv2.rectangle(subtract_mask, (0,0), (384, 450), 0, -1)
