...
# ====== 使用者參數設定 ======
folder_path = r"H:\droplet moving(high speed camera)\419\500hz-N001"
...
for filename in image_files:
    ...
    # 裁切區域：Y:320~550, X:1020~1180
    cropped_image = image[320:550, 1020:1180]
    ...
    _, binary_image = cv2.threshold(sobel_edges, 14, 255, cv2.THRESH_BINARY)
    ...
    if valid_contours:
        ...
        if M["m00"] != 0:
            ...
            # 儲存到列表
            centroid_list.append([filename, new_relative_X_mm, new_relative_Y_mm])
...
# 輸出 Excel
df = pd.DataFrame(centroid_list, columns=["Filename", "X_mm", "Y_mm"])
