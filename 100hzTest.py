...
# # === User-defined paths ===
# video_path = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\100 Hz\方形電極moving 100Hz.avi"
# output_folder = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\100 Hz\output" 
# frames_folder = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\100 Hz\frames"
...
#     # # Process the frame (e.g., grayscale)
#     # gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)

#     # Optional: save the frame
#     frame_filename = os.path.join(frames_folder, f'frame_{frame_num:04d}.png')
#     cv.imwrite(frame_filename, frame)

#     # Optional: display (slow)
...
img_path = r"C:\Users\wjrwe\Documents\NTU2025ImageProcessing\image processing practice\100 Hz\frames\frame_0678.png"
...
# 挑选主要边缘，生成黑白图像
_, binary_image = cv.threshold(sobel_edges, 7, 255, cv.THRESH_BINARY)
...
if largest_contour is not None:
    epsilon = 0.01 * cv.arcLength(largest_contour, True)  # 1% of arc length
    approx_contour = cv.approxPolyDP(largest_contour, epsilon, True)
    # 可视化简化后的轮廓
    approx_mask = np.zeros_like(cropped_image)
    cv.drawContours(approx_mask, [approx_contour], -1, 255, 1)
    cv.imshow("approxPolyDP contour", approx_mask)
    cv.imwrite(os.path.join(save_dir, '06c_approxPolyDP_contour.png'), approx_mask)
    cv.waitKey(0)
    cv.destroyAllWindows()
else:
    approx_contour = None
